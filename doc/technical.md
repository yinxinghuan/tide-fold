...
- `product.ts`：折叠状态机、真实效果幽灵演示、Pointer 输入、双语提示、音频与 WebGPU 错误恢复。
...
产品模式把 `TideFoldExperience.targetRatio` 接到原 `realBoxSize[2]`；向内闭合仍沿用上游每帧最大 `-0.007`，向外恢复限为 `+0.018`。控制器只改变这个上游参数，不写粒子位置、不改 shader。状态为 `loading → guide/idle → folding/ready → release → recovery → idle`，幽灵与真实 Pointer 输入共用 `setFold()`，因此引导必然驱动物理效果。
